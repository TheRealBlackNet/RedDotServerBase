...
func writeXmlLine(request: HttpRequest,\
	...
	if nodeType == XMLParser.NodeType.NODE_COMMENT:
		...
		for child in get_children():
			retval += child.writeXmlLine(request, response, false)
		# END TAG
		retval += "</" + tagName + ">"
...
func saveXml(path:String) -> void:
	var fileContent:String = self.writeXmlLine(null, null)
...
static func simpleFormat(xml:String) -> String:
	return xml.replace(">",">\r\n").replace("<!--","\r\n<!--")
